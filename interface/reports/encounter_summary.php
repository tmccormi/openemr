<?php
    ...
    // Used to report encounter and summary medical, short version of superbills
    //
    //@TODO htmlspecialchars on $POST data
	...
	if ($_POST['form_refresh']) {
		...
		$sql = "SELECT 
			...
			insurance_data.policy_number AS insurance_policy,
			GROUP_CONCAT(DISTINCT billing.code SEPARATOR ', ') AS CPT4_ICD9,
            GROUP_CONCAT(DISTINCT form_encounter.reason SEPARATOR ', ') AS medical_note,
			GROUP_CONCAT(DISTINCT billing.encounter SEPARATOR '<br>') AS enc
			FROM billing 
			INNER JOIN patient_data ON patient_data.pid = billing.pid 
			...
			INNER JOIN form_encounter ON form_encounter.pid = billing.pid
        ...
        // TAB DELIMITED (this should use a php csv function, really)
        
	if ($_POST['form_refresh'] == "export"){
		$result = sqlStatement($sql, $sqlBindArray);
		
		$out = "First/Last Name\tSex\tDOB\tSS\tDOS\tStreet/State/Zip\tInsurance Company\tPolicy#\tCPT Codes/ICD9 Codes\tMedical Note\n";
		
		while($row = sqlFetchArray($result))
		{
			$out .= $row['patient_name'] . "\t";
                        $out .= $row['sex'] . "\t";
                        $out .= $row['DOB'] . "\t";
                        $out .= $row['SS'] . "\t";
                        $out .= $row['DOS'] . "\t";
                        $out .= $row['address'] . "\t";
                        $out .= $row['insurance_company_name'] . "\t";
                        $out .= $row['insurance_policy'] . "\t";
                        $out .= $row['CPT4_ICD9'] . "\t";
                        $out .= $row['medical_note'] . "\n";
		}
		...
		header("Content-Disposition: attachment; filename=encounter_summary_".$dt_str.".csv");
		echo $out;
		exit;
	}
?>
		...
		<tr>
			<td>
				<div style='margin-left:15px'>
					...
					<a href='#' title='<?php echo htmlspecialchars(xl('Download Tab Delimited File'), ENT_QUOTES) ?>' class='css_button' onclick='$("#form_refresh").attr("value","export"); $("#theform").submit();'>
						<span>
			...
			<table>
				...
				<?php
					...
					<tr>
						...
						<form>
							<strong><?php echo htmlspecialchars(xl('Medical Note'),ENT_QUOTES) ?>:</strong><br>  <textarea name="" cols="100" rows="3" readonly="readonly"><?php echo $row['medical_note']?></textarea>
						</form>
						</td>
					</tr>
				<?php
