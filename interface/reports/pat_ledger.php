...
require_once($GLOBALS['srcdir'].'/wmt/printfacility.class.php');
require_once($GLOBALS['srcdir'].'/wmt/printpat.class.php');
...
function GetAllUnapplied($pat='') {
  $all = array();
	if(!$pat) return($all);
  $sql = "SELECT ar_session.*, ar_activity.encounter, ins.name FROM ".
      "ar_session LEFT JOIN ar_activity USING (session_id) ".
      "LEFT JOIN insurance_companies AS ins on ar_session.payer_id = ins.id ".
      "WHERE (encounter=0 OR encounter IS NULL) AND ar_session.patient_id=?";
  $result = sqlStatement($sql, array($pat));
  $iter = 0;
...
function PrintEncHeader($dt, $rsn, $dr) {
	...
	echo "<td colspan='5'><span class='bold'>Provider: </span>".
			"<span class='detail'>".text(UserIdLook($dr))."</span></td>";
 	echo "</tr>\n";	
	$orow++;
}
...
function PrintCreditDetail($detail, $pat, $unassigned=false) {
  ...
  foreach($detail as $pmt) {
    ...
    if($unassigned) {
		  ...
		  $enc_pmt = $enc_pmt + $pmt['pay_total'];
		  $total_pmt = $total_pmt + $pmt['pay_total'];
    } else {
...
if ($_REQUEST['form_refresh'] || $_REQUEST['form_csvexport']) {
  ...
  if ($INTEGRATED_AR) {
    ...
    if ($_REQUEST['form_csvexport']) {
      ...
      $facility = wmtPrintFacility::getFacility($form_facility);
      $patient = wmtPrintPat::getPatient($form_patient);
    
?>
<div id="report_header">
<table width="98%"  border="0" cellspacing="0" cellpadding="0">
	<tr>
		<td class="wmtPrnChapter">Patient Ledger</td>
	</tr>
  <tr>
    <td class="wmtPrnChapter"><?php echo $facility->facility; ?></td>
  </tr>
  <tr>
    <td class="wmtPrnChapter"><?php echo $facility->addr; ?></td>
  </tr>
  <tr>
    <td class="wmtPrnChapter"><?php echo $facility->csz; ?></td>
  </tr>
	<tr>
		<?php 
			$title = 'All Providers';
		...
		?>
    <td class="wmtPrnChapter"><?php echo $title; ?></td>
	</tr>
	<tr>
		<?php 
			$title = 'For Dates: '.$from_date.' through '.$to_date;
		?>
    <td class="wmtPrnChapter"><?php echo $title; ?></td>
	</tr>
</table>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <td class="wmtPrnLabel">ID:</td>
		<td class="wmtPrnBody"><?php echo $form_patient;?></td>
  </tr>
</table>
</div>
...
<table >
 <thead>
  ...
  <th><?php xl('Charge','e'); ?></th>
	<th><?php xl('Payment','e'); ?></th>
 ...
 <?php
    ...
    while ($erow = sqlFetchArray($res)) {
      ...
      $csv = '';
			...
			if($erow['encounter'] != $prev_encounter_id) {
				...
				$enc_chg = $enc_pmt = $enc_adj = $enc_bal = 0;
			}
			if($erow{'id'}) {
    ...
    // This is the end of the encounter/charge loop - 
		$uac = GetAllUnapplied($form_patient);
		if(count($uac) > 0) {
			if($orow) {
        $bgcolor = (($bgcolor == "#FFFFDD") ? "#FFDDDD" : "#FFFFDD");
