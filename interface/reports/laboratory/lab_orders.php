...
if ($_POST['form_refresh'] || $_POST['form_orderby']) {
	...
	if ($generic !== FALSE) {
		...
		$query1 .= "fo.id, fo.pid, fo.status AS order_status, fo.order_number, fo.request_billing, ";
	...
	if ($quest !== FALSE) {
		...
		$query2 .= "fo.id, fo.pid, fo.status AS order_status, fo.order_number, fo.request_billing, ";
	...
	if ($labcorp !== FALSE) {
		...
		$query3 .= "fo.id, fo.pid, fo.status AS order_status, fo.order_number, fo.request_billing, ";
...
<head>
		...
		<title><?php xl('Lab Orders Report','e'); ?></title>
...
<?php
		...
		while ($row = sqlFetchArray($results)) {
			...
			if ($form_status && $form_status != 'g' && $form_status != $ostatus) continue; // wrong order status
