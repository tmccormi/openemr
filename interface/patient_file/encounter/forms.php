...
// 
// Add E-Sign and Amendment options 2013-08
...
require_once("$srcdir/esign/ESign.class.php");
?>
<html>
    ...
    <head>
        ...
        <script src="<?php echo $GLOBALS['webroot'] ?>/library/esign/js/esign.js"></script>
        <style type="text/css">@import url(<?php echo $GLOBALS['webroot'] ?>/library/esign/style/esign.css);</style>
...
?>
            ...
            <div style='float:left'>
                ...
                if (is_numeric($pid)) {
                    ...
                    echo htmlspecialchars(xl('for', '', ' ', ' ') . $result['lname'] . ", " . $result['fname']);
...
<?php } ?>
    ...
    <?php } else { ?>
            ...
            <?php
            $result = getFormByEncounter($pid, $encounter, "id, date, form_id, form_name, formdir, user, deleted");
            if ($result) {
                ...
                foreach ($result as $iter) {
                    ...
                    if ($formdir == 'newpatient') {
                         ...
                         xl_form_title($iter['form_name']);
                         $form_name = $formdir;
                    }

                    echo "<tr>";
                    echo "<td style='border-bottom:1px solid'>";
                    // build links to edit/sign/view or amend the form
                    echo "<div class='form_header_controls'>";

                    $res2 = sqlFetchArray(sqlStatement("SELECT * FROM eSignatures where tid=" . $iter['form_id'] . " AND `table`='forms_" . $formdir . "' AND signed = '1'"));

                    if ($res2['id'] == '') {
                        
                        // Get the table used by the form 
                        $registry = sqlFetchArray(sqlStatement("select * from registry where directory = '". $formdir ."'"));
                        $esign = new ESign();
                        $esign->init($iter['form_id'], $registry['table_name']);
                        
                        //Edit Form
                        if (($GLOBALS['esign_individual'] && $GLOBALS['lock_esign_individual'] && $esign->hasSignature()) ||
                               ($GLOBALS['esign_all'] && $formdir === 'newpatient' && $GLOBALS['lock_esign_all'] && $esign->hasSignature())){ 
                            echo "<a target='" .
                            ($GLOBALS['concurrent_layout'] ? "_parent" : "Main") .
                            "' href='$rootdir/patient_file/encounter/view_form.php?" .
                            "formname=" . $form_name . "&id=" . $iter['form_id'] .
                            "' onclick='top.restoreSession()' id='edit' class='css_button_small'><span>" . xl('Read-Only') . "</span></a>";
                        }
                        else
                        {
                            echo "<a target='" .
                            ...
                            "' href='$rootdir/patient_file/encounter/view_form.php?" .
                            "formname=" . $form_name . "&id=" . $iter['form_id'] .
                            "' onclick='top.restoreSession()' id='edit' class='css_button_small'><span>" . xl('Edit') . "</span></a>";
                        }
                        
                        // eSign Form
                        
                        if ($GLOBALS['esign_individual']  || ($GLOBALS['esign_all'] && $formdir === 'newpatient' )){
                                
                            echo "<a target='" .
                                ($GLOBALS['concurrent_layout'] ? "_parent" : "Main") .
                                "' href='#mask_content" .
                                "' name='esign_modal" . 
                                "' onclick='esign.getESignaturePrompt(this); return false;".
                                "' id='esign' class='css_button_small".
                                "' formdir='" . $formdir . 
                                "' formid='" . $iter['form_id'] . 
                                "' ><span>"
                                . xl('eSign') . "</span></a>";
                    ...
                    } else {  // forms are signed and can be amended @todo deal with signed/but not locked
                        ...
                        $button.= "' id='amend' class='css_button_small iframe' ><span>"
                    ...
                    if (acl_check('admin', 'super')) {
                        if ($formdir != 'newpatient') {
                            ...
                            if ($GLOBALS['esign_individual'] || ( $GLOBALS['esign_all'] && $formdir === 'newpatient' )) {
                                //todo:  add in a check to see if esigning is turned on for this object
                                //  if so,  display the esignature log block here
                                //  Stub code:
                                echo "<hr>";
                                echo "<div id='".$formdir."_".$iter['form_id']."_signature_log'></div>";
                                echo "<script>esign.getESignatureLog('".$formdir."','".$iter['form_id']."');</script>";
                                // end display of esignature log block
                            }
...
<?php } ?>
        ...
        <!-- jquery popup content pane -->
        <div id='mask_content' class="window rounded"></div>
        
        <!-- Mask to cover the whole screen for jquery popup -->
        <div id="mask"></div> 
